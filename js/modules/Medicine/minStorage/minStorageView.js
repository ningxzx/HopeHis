define(['txt!../../Medicine/minStorage/minStorage.html',
    ...
    function (Template, inStorageModel, minCheckModel, commonModel, Handlebars, backbone, jctLibs, bootstrapTable) {
        ...
        var cols2 = [
            ...
            {field: 'unit_price', width: "4%", title: '成本价', events: jctLibs.rUnitPrice, formatter: jctLibs.generatePrices},
            {field: 'sell_price', width: "4%", title: '售药价', events: jctLibs.rSellPrice, formatter: jctLibs.generatePrices},
            {field: 'pesc_price', title: '处方价', width: "6%", events: jctLibs.rPescPrice, formatter: jctLibs.generatePrices},
            {field: 'product_date_time', title: '生产日期', width: "12%", events: jctLibs.rProductTime, formatter: jctLibs.generateDate},
            {field: 'deadline_date_time', title: '过期日期', width: "12%", events: jctLibs.rDeadlineTime, formatter: jctLibs.generateDate},
        ...
        var view = Backbone.View.extend({
            initialize: function () {
                ...
                this.listenTo(this.inStorageModel, "searchOwnDrug", this.renderOwnDrug);
            },
            events: {
            ...
            searchCheck:function () {
                ...
                this.inStorageModel.getOwnCheckRecord(data);
            },
            ...
            searchOwnMedicine:function () {
                var name = $('#un_own_name').val(),data={};
                if(name!==""){
                    data['drug_name']=name;
                }
                this.inStorageModel.searchOwnDrug(data);
            },
            renderOwnDrug:function(res){
                if(res.errorNo==0){
                    $(this.el).find("#own_drug_table").bootstrapTable('load',res.rows);
                }
                else{
                    $(this.el).find("#own_drug_table").bootstrapTable('load',[]);
                }
            },
            ...
            searchKeyOwnMedicine:function () {
                if (e.keyCode == '13') {
                    this.searchOwnMedicine();
                }
            },
            //关闭modal
            closeModal: function () {
            ...
            addOwnDrug:function () {
                var rows = $("#own_drug_table").bootstrapTable("getAllSelections");
                $("#medicine_modal").modal("close");
                rows.forEach(function (row) {
                    row['drug_code']=row['drug_id'];
                    row['chinese_name']=row['drug_name'];
                })
                this.addetailtb(rows);
                $("#own_drug_table").bootstrapTable("uncheckAll");
            },
            //提交明细表
            ...
            submitCheck: function () {
                ...
                //var dataDetail= $("#table_minStorage").bootstrapTable("getAllSelections");
            ...
            render: function () {
                ...
                $el.find("#own_drug_table").bootstrapTable({
                    columns: ownDrugColumn,
                    clickToSelect: true,
                });
                ...
                this.inStorageModel.searchOwnDrug();
